...
export default function App() {
  ...
  const handleSyncGoDaddy = async (companyId) => {
    ...
    try {
      ...
      else { alert(`Successfully synced ${data.count} domains from GoDaddy!`); fetchData(); }
  ...
  const handleToggleSubscribe = (task) => {
    ...
    if (task.id) { setTasks(tasks.map(t => t.id === task.id ? updatedTask : t)); sendToAPI('save_task', { ...updatedTask, actorId: currentUser.id }); }
  };
  ...
  const handleAddComment = () => {
    ...
    setCurrentTask(updatedTask); setNewCommentText('');
    if (updatedTask.id) {
  ...
  const openShowModal = (show = null, scope = 'episode') => {
    if (show) { const dayOfWeek = new Date(`${show.showDate}T12:00:00`).getDay(); setEditingShow({ ...show, userIds: show.userIds || [], editScope: scope, originalTitle: show.title, originalShowTime: show.showTime, originalDayOfWeek: dayOfWeek, revShare: show.revShare ?? 100 }); } 
  ...
  const handleArchiveSeries = (showToArchive) => {
    ...
    const episodesToArchive = shows.filter(s => {
        ...
        if (new Date(`${s.showDate}T12:00:00`).getDay() !== showToArchive.originalDayOfWeek) return false;
        return true;
    });
    ...
    setShows([...nonSeriesShows, ...updatedEpisodes]); sendToAPI('save_shows_batch', { shows: updatedEpisodes }); logActivity('Shows', 'Series Archived', `Archived series "${showToArchive.originalTitle}"`); setIsShowModalOpen(false);
  };
  const handleSaveShow = (e) => {
    ...
    if (isNew && editingShow.isRecurring) {
        const newShows = []; const baseDate = new Date(`${editingShow.showDate}T12:00:00`); 
        const numOccurrences = parseInt(editingShow.occurrences) === 0 ? 260 : (parseInt(editingShow.occurrences) || 1);
        for (let i = 0; i < numOccurrences; i++) {
            const iterDate = new Date(baseDate); iterDate.setDate(iterDate.getDate() + (i * 7));
        ...
        setShows(prev => [...prev, ...newShows]); sendToAPI('save_shows_batch', { shows: newShows });
  ...
  const handleDeleteShow = (id) => {
    ...
    setIsShowModalOpen(false); sendToAPI('delete_show', { id });
  };
  ...
  const handleSaveAnalyticsProperty = (e) => {
      ...
      if (editingAnalyticsProperty.id) localStorage.setItem('pendingGaId', editingAnalyticsProperty.id); else localStorage.removeItem('pendingGaId');
  ...
  const handleSaveSponsorship = (e) => {
    ...
    setIsSponsorshipModalOpen(false); sendToAPI('save_sponsorship', spData);
  };
  const handleDeleteSponsorship = (id) => {
    ...
    setIsSponsorshipModalOpen(false); sendToAPI('delete_sponsorship', { id });
  };
  ...
  const handleSaveContact = (e) => {
    ...
    setIsContactModalOpen(false); sendToAPI('save_contact', ctData);
  };
  const handleDeleteContact = (id) => {
    const ct = contacts.find(c => c.id === id);
    if (ct) logActivity('CRM', 'Contact Deleted', `Deleted contact "${ct.name}"`);
    setContacts(contacts.filter(c => c.id !== id));
    setIsContactModalOpen(false); sendToAPI('delete_contact', { id });
  };
  ...
  const handleSavePassword = (e) => {
    ...
    setIsPasswordModalOpen(false); sendToAPI('save_password', pwData);
  };
  const handleDeletePassword = (id) => {
    ...
    setIsPasswordModalOpen(false); sendToAPI('delete_password', { id });
  };
  ...
  const handleSaveTeamMember = (e) => {
    ...
    setEditingTeamMember(null); setIsTeamModalOpen(false);
  };
  const handleDeleteUser = (userId) => {
    ...
    setIsTeamModalOpen(false); sendToAPI('delete_user', { id: userId });
  };
  ...
  const handleSaveCompany = (e) => {
    ...
    setIsCompanyModalOpen(false); sendToAPI('save_company', { ...companyData, notifyNewUsers: newlyAddedUserIds });
  };
  ...
  const handleRestoreProject = (project) => {
     ...
     setProjects(projects.map(p => p.id === project.id ? updated : p));
     sendToAPI('save_project', updated); setIsProjectModalOpen(false);
  };
  const handlePermanentDeleteProject = (projectId) => {
     ...
     setIsProjectModalOpen(false); sendToAPI('delete_project', { id: projectId });
  };
  ...
  const handleSaveYoutubeChannel = (e) => {
    ...
    if (editingYoutubeChannel.id) localStorage.setItem('pendingYtId', editingYoutubeChannel.id); else localStorage.removeItem('pendingYtId');
    const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${scope}&access_type=offline&prompt=consent`;
    setIsYoutubeModalOpen(false); window.location.href = authUrl;
  };
  const handleUpdateYoutubeChannel = (e) => {
    ...
    setIsYoutubeModalOpen(false); sendToAPI('update_youtube_channel', editingYoutubeChannel);
  };
  const handleDeleteYoutubeChannel = (channelId) => {
    ...
    setIsYoutubeModalOpen(false); sendToAPI('delete_youtube_channel', { id: channelId });
  };
  ...
  const handleSaveSpreakerShow = (e) => {
    ...
    setIsSpreakerModalOpen(false); window.location.href = authUrl; 
  };
