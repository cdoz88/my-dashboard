  ...
  const AuthScreen = () => {
    ...
    const handleAuth = async (e) => {
      ...
      if (isRegistering) {
        ...
        const newUser = { id: 'u' + Date.now(), name, email: email.toLowerCase(), password, isAdmin: isFirstUser, canViewProjects: true, canViewBudget: isFirstUser, canViewDomains: isFirstUser, canViewEvents: isFirstUser, canViewSpreaker: isFirstUser, avatarUrl: '' };
        ...
        try {
          const response = await fetch(`${API_URL}?action=login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email.toLowerCase(), password }) });
          const data = await response.json();
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex overflow-hidden">
            ...
            <div className="w-1/3 border-r border-slate-100 bg-slate-50 flex flex-col">
              <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-white">
                ...
                <button onClick={() => setEditingTeamMember({ id: null, name: '', email: '', password: '', isAdmin: false, canViewProjects: true, canViewBudget: false, canViewDomains: false, canViewEvents: true, canViewSpreaker: false })} className="text-blue-600 hover:bg-blue-50 p-1.5 rounded-lg transition-colors"><Plus size={18}/></button>
              </div>
