...
export default function LedgerDashboard({
  ...
  const grandTotalPaid = payouts.reduce((sum, p) => sum + parseFloat(p.amount || 0), 0);
  const grandTotalOwed = grandTotalEarned - grandTotalPaid;
    ...
    <div className="p-4 sm:p-8 h-full flex flex-col w-full bg-slate-50/50 overflow-y-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
           ...
           >
             <Plus size={16} strokeWidth={2.5} /> Log Payment
           </button>
        </div>
      </div>
                   ...
                   <table className="w-full text-left min-w-[1000px]">
                       <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                           <tr className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                               ...
                               <th className="p-4 w-32 text-right">Total Paid</th>
                           ...
                           {eligibleShows.sort((a,b) => a.title.localeCompare(b.title)).map(show => {
                               ...
                               return (
                                   <tr key={show.id} className="hover:bg-slate-50 transition-colors">
                                       ...
                                       <td className="p-4 text-right font-medium text-slate-700">{formatCurrency(paid)}</td>
                                       <td className="p-4 text-right font-bold text-emerald-600 bg-emerald-50/30">{formatCurrency(Math.max(0, balance))}</td>
                                       <td className="p-4 text-center">
                                           <button onClick={() => openPayoutModal({ showId: show.id, amount: Math.max(0, balance), paymentDate: new Date().toISOString().split('T')[0] })} className="text-[10px] font-bold text-white bg-slate-800 px-2 py-1 rounded hover:bg-slate-700 transition-colors whitespace-nowrap">
                                               Pay Now
                                           </button>
                                       </td>
                                   </tr>
                           ...
                           {payouts.map(p => {
                               ...
                               return (
                                   <tr key={p.id} className="hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => openPayoutModal(p)}>
                                       ...
                                       <td className="p-4 text-right font-bold text-emerald-600">{formatCurrency(p.amount)}</td>
                                       <td className="p-4">
