      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b border-slate-100 flex-shrink-0">
          <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
            ...
            {editingSpreakerShow.id ? 'Edit Show' : 'Add New Show'}
          </h3>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
              <p className="text-sm text-slate-600">
                Clicking the button below will redirect you to Spreaker to securely authorize access to your podcast statistics. You will automatically return here once connected.
              </p>
            </div>
          </div>
        </form>
        <div className="p-6 mt-2 flex justify-end gap-3 border-t border-slate-100 flex-shrink-0">
          ...
          <button type="submit" form="spreakerForm" className="px-4 py-2 bg-[#ffc005] hover:bg-[#e6ad04] text-slate-900 rounded-lg font-bold">{editingSpreakerShow.id ? 'Reconnect Spreaker' : 'Connect Account'}</button>
        </div>
      </div>
    </div>
  );
