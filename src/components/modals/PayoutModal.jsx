      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden border-t-4 border-t-emerald-500">
        <div className="flex justify-between items-center p-6 border-b border-slate-100 flex-shrink-0">
          <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
            ...
            {editingPayout.id ? 'Edit Payment Record' : 'Log New Payout'}
          </h3>
        ...
        <div className="overflow-y-auto flex-1 p-6">
          <form id="payoutForm" onSubmit={handleSavePayout} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input required type="number" step="0.01" min="0.01" value={editingPayout.amount} onChange={(e) => setEditingPayout({...editingPayout, amount: e.target.value})} className="w-full pl-8 pr-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold text-emerald-700" placeholder="0.00" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1 flex items-center gap-1.5"><Calendar size={14} className="text-slate-400"/> Date Sent</label>
            ...
            <div className="pt-2 border-t border-slate-100">
              ...
              <textarea rows="3" value={editingPayout.notes || ''} onChange={(e) => setEditingPayout({...editingPayout, notes: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="e.g., Payment for January & February watch hours..." />
            </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 flex-shrink-0">
          ...
          <button type="submit" form="payoutForm" className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors font-bold shadow-sm">
             {editingPayout.id ? 'Update Record' : 'Log Payment'}
          </button>
        </div>
      </div>
    </div>
