...
export default function PayoutModal({
  ...
  // Failsafe: Format trailing decimals immediately if they snuck in through the state
  useEffect(() => {
    if (editingPayout && editingPayout.amount) {
        const rawAmount = String(editingPayout.amount);
  ...
  // Block users from typing more than 2 decimals mathematically
  const handleAmountChange = (e) => {
      ...
      setEditingPayout({...editingPayout, amount: val});
  };
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input required type="number" step="0.01" min="0.01" value={editingPayout.amount} onChange={handleAmountChange} className={`w-full pl-8 pr-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold ${editingPayout.transactionType === 'Deduction' ? 'text-red-600' : 'text-emerald-700'}`} placeholder="0.00" />
                </div>
              </div>
              <div>
