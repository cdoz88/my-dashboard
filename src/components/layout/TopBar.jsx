...
import { 
  ...
  RefreshCw, Upload, Kanban, Contact, Activity, Tv, Award, Network, LayoutGrid, BookUser, LayoutPanelTop, Lock, Camera, Calculator
} from 'lucide-react';
...
export default function TopBar({
  ...
  const isShowsView = currentApp === 'shows';
        ...
        {isAppSwitcherOpen && (
          <>
            ...
            <div className="absolute top-full left-0 mt-1 w-56 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-50 py-1">
              ...
              {(currentUser.isAdmin) && <button onClick={() => { setCurrentApp('ledger'); setIsAppSwitcherOpen(false); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium transition-colors border-b border-slate-100 ${currentApp === 'ledger' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700 hover:bg-slate-50'}`}><div className={`p-1.5 rounded-md ${currentApp === 'ledger' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}><Calculator size={18} /></div>Creator Ledger</button>}
